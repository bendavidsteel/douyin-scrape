...
def main():
    df = pl.read_parquet('./data/douyin_related_videos.parquet.zstd', columns=['aweme_id', 'desc', 'create_time', 'region', 'statistics'])
    ...
    clusterer.fit(clusterable_vectors=umap_vectors, embedding_vectors=embeddings, object_to_text_function=llm.translate_text)
    ...
    topic_df = df.with_columns([pl.Series(name=f"cluster_layer_{i}", values=c.topic_name_vector) for i, c in enumerate(topic_model.cluster_layers_)])\
        .with_columns(pl.Series(name='umap_vector', values=umap_vectors))
